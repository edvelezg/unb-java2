...
public class Quicksort {
	...
	private static <T extends Comparable<T>> String printArray(T[] array, int left, int right, int lo) {
		...
		for (int i = 0; i < array.length; i++) {
			...
			if (i == lo)
				buf.append(String.format("%4s", String.format("(%d)", array[lo])));
	...
	private static <T extends Comparable<T>> int splitList(T[] list, int lo, int hi) {
		// System.out.println("splitList(list=" + Arrays.toString(list)+" lo="+lo+"
		// hi="+hi+") pivot: " + list[lo]);
		T pivot = list[lo];
		int left = lo + 1;
		int right = hi;
	...
	private static <T extends Comparable<T>> void swap(T[] list, int left, int right) {
		// System.out.println("SWAP (list="+ Arrays.toString(list)+" left="+left+"
		// right="+right+")");
		if (left != right) {
			T temp = list[left];
	...
	private static <T extends Comparable<T>> void sort(T[] list, int lo, int hi) {
		// System.out.println("sort(list="+ Arrays.toString(list)+" lo="+lo+"
		// hi="+hi+")");
		// base case:
		if (hi - lo <= 0) {
			return;
